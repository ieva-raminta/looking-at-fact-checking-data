...
for item in _items:
    ...
    claim = result["claim"]
    sections_labels = [
        {section_key: result["labels"][section_key]}
        for section_key in result["labels"].keys()
    ]
    sections_texts = [
        {section_key: result["text"][section_key]}
        for section_key in result["labels"].keys()
    ]
    section_labels_for_claim = [
        {list(section.keys())[0]: list(section.values())[0]["section_label"]}
        for section in sections_labels
    ]
    conflicting_evidence = "supported" in [
        list(section.values())[0] for section in section_labels_for_claim
    ] and "refuted" in [
        list(section.values())[0] for section in section_labels_for_claim
    ]
    if conflicting_evidence:
        ...
        sentence_labels = [
            {list(section.keys())[0]: list(section.values())[0]["sentence_labels"]}
            for section in sections_labels
        ]
        refuting_sentence_ids_dict = {}
        supporting_sentence_ids_dict = {}
        refuting_sentence_ids = [
            {
                list(section.keys())[0]: [
                    value_id
                    for value_id, value in enumerate(list(section.values())[0])
                    if value == "refuting"
                ]
            }
            for section in sentence_labels
        ]
        ...
        for key in refuting_sentence_ids_dict:
            refuting_sentences.extend(
                [
                    sections_texts_dict[key][id_]
                    for id_ in refuting_sentence_ids_dict[key]
                ]
            )
        for key in supporting_sentence_ids_dict:
            supporting_sentences.extend(
                [
                    sections_texts_dict[key][id_]
                    for id_ in supporting_sentence_ids_dict[key]
                ]
            )
